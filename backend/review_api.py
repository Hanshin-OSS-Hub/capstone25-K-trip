...
try:
    ...
    logger_init_msg = "Google Translate client initialized successfully"
except Exception as _e:
    translate_client = None
    logger_init_msg = f"Failed to initialize Google Translate client: {str(_e)}"
...
app = FastAPI(title="Review API", version="2.0.0")
...
def save_upload_file(file: UploadFile, media_type: str) -> tuple[str, int]:
    suffix = Path(file.filename).suffix.lower() if file.filename else ""

    if media_type == "photo" and suffix not in ALLOWED_IMAGE_EXTENSIONS:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid image format. Allowed: {', '.join(ALLOWED_IMAGE_EXTENSIONS)}"
        )
    if media_type == "video" and suffix not in ALLOWED_VIDEO_EXTENSIONS:
        raise HTTPException(
            status_code=400,
    ...
    media_url = f"/uploads/reviews/{unique_filename}"
    return media_url, file_size

def get_or_create_translation(conn, review_id: int, language: str, title: Optional[str], comment: str) -> Optional[dict]:
    cursor = conn.cursor(dictionary=True)
    try:
        cursor.execute(
            """
            SELECT translated_title, translated_comment
            FROM review_translations
            WHERE review_id = %s AND language = %s
        ...
        translated_comment = translate_client.translate(comment, target_language=language)['translatedText']
        translated_title = None
        if title:
            translated_title = translate_client.translate(title, target_language=language)['translatedText']

        cursor.execute(
            """
            INSERT INTO review_translations
                (review_id, language, translated_title, translated_comment, translation_engine, is_auto)
            VALUES (%s, %s, %s, %s, 'google', TRUE)
            """,
            (review_id, language, translated_title, translated_comment)
        )
        conn.commit()
        ...
        logger.warning(f"Translation failed for review {review_id}: {str(e)}")
        return None
    finally:
        cursor.close()
    ...
    try:
        ...
        if language:
            translation = get_or_create_translation(
                conn, review_id, language.value,
                review.get('review_title'), review.get('review_comment', '')
            )
            if translation:
    ...
    try:
        ...
        if language and reviews:
            for review in reviews:
                translation = get_or_create_translation(
                    conn, review['review_id'], language.value,
                    review.get('review_title'), review.get('review_comment', '')
                )
                if translation:
                    review['translated_title'] = translation['translated_title']
...
async def upload_review_photo(
    ...
    try:
        ...
        return {"message": "Photo uploaded successfully", "media_id": media_id, "media_url": media_url, "file_size": file_size}
    except HTTPException:
        raise
    except Error as e:
        conn.rollback()
...
async def upload_review_video(
    ...
    try:
        ...
        return {"message": "Video uploaded successfully", "media_id": media_id, "media_url": media_url, "file_size": file_size}
    except HTTPException:
        raise
    except Error as e:
        conn.rollback()
...
def add_review_media(review_id: int, media: ReviewMediaCreate, user_id: int = Query(...), conn = Depends(get_db)):
    ...
    try:
        ...
        return {"message": "Media added successfully", "media_id": media_id}
    except HTTPException:
        raise
    except Error as e:
        conn.rollback()
